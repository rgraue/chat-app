...
export const Navbar = ({
  ...
  workspaceSetter: React.Dispatch<React.SetStateAction<boolean>>;
}) => {
  ...
  return (
    ...
    >
      <Flex
        direction={device == "BROWSER" ? "row" : "column-reverse"}
      ...
      >
        <Flex direction={"row"} alignItems={"center"} gap={"2"} w={"25%"}>
          ...
          {isError && (
            <Tooltip content={"No connection to model"}>
              <FiCloudOff />
            </Tooltip>
          )}
        </Flex>
        ...
        {device == "BROWSER" && (
          <Flex w={"25%"} direction={"row-reverse"}>
            <ColorModeButton />
          </Flex>
        )}
      </Flex>
