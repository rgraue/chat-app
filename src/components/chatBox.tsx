...
export const ChatBox = ({
  ...
  const addDialog = () => {
    ...
    setDialog((old) => [
      ...old,
      <Dialog text={prompt} side={"RIGHT"} key={rightUUID} />,
    ]);
    ...
    mutate({
      ...
      conversation: activeWorkspace
        ? workspaces.getConversation(activeWorkspace)
        : undefined,
    });
    ...
    setDialog((old) => [
      ...old,
      <Dialog text={"..."} side={"LEFT"} key={leftUUID} />,
    ]);
    setPrompt("");
  };
    ...
    <Container width={device == "MOBILE" ? "3/4" : "3/5"}>
      <Container borderWidth={1} borderColor={"gray"} borderRadius={10}>
        <ScrollArea.Root height="70vh">
          ...
          <ScrollArea.Viewport>
            {/* @ts-expect-error chakra is whack*/}
            <ScrollArea.Content>{dialog}</ScrollArea.Content>
          </ScrollArea.Viewport>
