...
export class WorkspaceService {
    ...
    saveWorkspaces () {
        const serialized = btoa(JSON.stringify({convos: this.conversations, active: this.activeConversation}));
    ...
    getConversations () {
        return new Set(
            this.conversations.map(x => x.conversationName)
        )
    }
    ...
    getConversation (conversation: string) {
        return this.conversations.filter(message => message.conversationName == conversation && message.role != 'init');
    }
    ...
    clearConversation (toClear: string) {
        this.conversations = this.conversations.filter(message => message.conversationName != toClear);
